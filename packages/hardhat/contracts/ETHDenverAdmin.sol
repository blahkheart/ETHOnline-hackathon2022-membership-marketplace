...
contract ETHDenverAdmin is AccessControl, Ownable{
    event NewOrder(string id, address indexed vendorId, address indexed userId, uint256 indexed amount, uint256 time, bool done);
    event DoneOrder(string id, address indexed vendorId, address indexed userId, uint256 indexed amount, uint256 time, bool done);
    event Register(address indexed userId, uint256 time, bool isVendor);
    ...
    function register(address _userId, bool _isVendor)public {
        ...
        // buidlBuxx.mintToken(msg.sender, (newLevel * baseLevelUpReward) * 1e18);
        emit Register(_userId, block.timestamp, _isVendor);
    }
